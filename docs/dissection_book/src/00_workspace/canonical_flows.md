...
Steps:
1. Parse CLI options with `cli_support` and shared helpers in `colon_sim_tools::services`.
...
Notes:
- `colon_sim_tools` is slated for split/trim; app-specific bins should move to the app repo.
- Keep `colon_sim_tools` unpublished; shared bits can migrate into other crates over time.
