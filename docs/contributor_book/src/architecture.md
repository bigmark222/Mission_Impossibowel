...
- **Apps**: live in their own repository (reference `colon_sim`, `hello_substrate` demo, and any domain-specific bins). Apps supply domain systems; substrate stays detector- and domain-agnostic.
- **Entrypoints**: app repos own the binaries/CLI; this repo ships shared crates only.

## Runtime flow (per run)
1. App CLI (in the app repo) parses args → choose `RunMode` (Sim/Datagen/Inference) → build the app.
...
- **Capture** (`sim_view` interactive from the app repo or `datagen` headless) → writes `run_<ts>` dirs under `assets/datasets/captures/` with frames, labels, overlays (optional), manifest.
...
- **Inference** (live via `inference_view` in the app repo or single-image) loads checkpoint; falls back to heuristic detector if none provided.
