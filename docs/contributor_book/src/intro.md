...
- Release/publishing: versioning, publish order.
- Roadmap/migration: upcoming changes (colon_sim_tools split, publish plan).

Housekeeping:
- Build this book: `mdbook build docs/contributor_book`
- User-facing flows live in the app repos (legacy user book retired).
- Current release target: crates at `0.1.1`; burn-core 0.14.1 is fixed and no patch is required.
