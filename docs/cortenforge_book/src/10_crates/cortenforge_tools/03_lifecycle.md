...
- App-facing bins (`datagen`, `datagen_scheduler`, `tui`) are gated by features; planned to move to app repo. `gpu_probe` is shared; `gpu_macos_helper` is a deprecated alias (remove in v0.3+).
